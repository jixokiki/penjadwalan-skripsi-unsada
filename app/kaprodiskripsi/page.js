...
//JANGAN DIHAPUSS YAA IKIIIIIIII OIII INI JANGAN DIHAPUS
// // KaprodiPage.jsx
// "use client";
// import { useState, useEffect } from "react";
// import { auth, db } from "@/lib/firebase";
...
//JANGAN DIHAPUS YA IKI INI CODE KAPRODI SKRIPSI YANG BENER TINGGAL DEBUGGING SAJA 
// // KaprodiPage.jsx
// "use client"
...
//   if (selectedCategory === "Sempro") {
//     dataset = dataset.filter(m => m.statusSempro === "Lagi Sidang Seminar Isi");
//     if (dataset.length === 0) {
//       alert("Tidak ada mahasiswa Sempro dengan status 'Lagi Sidang Seminar Isi'");
//       return;
//     }
//   }
...
//     if (userSnap.exists()) {
//       const userData = userSnap.data();
//       if (userData[statusField] === "Lagi Sidang Seminar Isi") {
//         console.log(`✅ Mengubah status ${statusField} untuk ID ${jadwal.mahasiswaId}`);
//         await updateDoc(userRef, {
//           [statusField]: "Lagi Sidang Skripsi",
//         });
//       } else {
//   console.warn(`⛔ Dilewati: ${statusField} sekarang = ${userData[statusField]}`);
// }
...
const KaprodiPage = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState("Skripsi");
  ...
  const fetchData = async () => {
    const semproSnap = await getDocs(collection(db, "usersSempro"));
    const seminarSnap = await getDocs(collection(db, "usersSkripsi"));
    const pembimbingSnap = await getDocs(collection(db, "dosen"));
    const pengujiSnap = await getDocs(collection(db, "penguji"));
    ...
    // </div>
     <motion.div className={styles.container} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
          <div className={styles.gridAnalyticBox}>
            {/* <div className={styles.card} onClick={() => setSelectedCategory("Sempro")}>📘 Mahasiswa Sempro ({dataSempro.length})</div> */}
            {/* <div className={styles.card} onClick={() => setSelectedCategory("SeminarIsi")}>📕 Seminar Isi ({dataSeminarIsi.length})</div> */}
            <div className={styles.card} onClick={() => setSelectedCategory("Skripsi")}>📗 Skripsi ({dataSempro.length})</div>
          </div>
