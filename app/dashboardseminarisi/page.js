...
import styles from "./dashboardseminarisi.module.scss";
...
export default function DashboardSeminarIsi() {
...
//JANGAN DIHAPUS YA IKI INI UPDATE YANG UDAH BENER CUMAN KURANG BEBERAPA AJA TGL 12 JULI 2025
// const handleRegister = async (e) => {
//   e.preventDefault();
//   setError(null);
//   setLoading(true);
...
const fetchUserDataByNim = async (inputId) => {
  ...
  try {
    const docRef = doc(db, "usersSeminarIsi", inputId);
    const docSnap = await getDoc(docRef);
    ...
    console.error("Gagal ambil data dari usersSeminarIsi:", error);
  ...
  return (
    <>
      ...
      <div className={styles.container}>
        <h1 className={styles.title}>Pengajuan Seminar Isi</h1>
...
<input
            ...
            <label>File TA1 (SEMINAR ISI)</label>
            <input
              type="file"
              name="fileTA1"
              onChange={handleFileChange}
